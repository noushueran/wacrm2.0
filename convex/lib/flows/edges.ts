 ...
 *
 * Convex port of `src/lib/flows/edges.ts` — ported verbatim except for
 * one import path: `BuilderNode` comes from the sibling `./shared.ts`
 * (a plain-data extraction of the original `@/components/flows/shared`
 * — see that file's header comment for why) instead of the original
 * `.tsx` component module.
 *
