 ...
 *
 * Convex port of `src/lib/flows/types.ts` — ported verbatim (this file
 * has zero imports, so there is nothing to adjust). Needed here because
 * `convex/lib/flows/fallback.ts` imports `DEFAULT_FALLBACK_POLICY`/
 * `FlowFallbackPolicy` from it; the automations/flows ENGINE (Phase 6,
 * Task 4) will need the rest (node configs, trigger configs) too, so the
 * whole file is ported rather than just the fallback-policy slice.
 *
 ...
 * references in JSONB.
 *
 * Note: `FlowRow`/`FlowNodeRow`/`FlowRunRow` below describe the
 * Supabase (snake_case) row shape the ORIGINAL engine read via
 * `supabaseAdmin`. The Convex flows engine (Task 4) reads
 * `Doc<"flows">`/`Doc<"flowNodes">`/`Doc<"flowRuns">` (camelCase)
 * instead — these three interfaces are kept for reference/parity with
 * the ported source but are not expected to be instantiated on the
 * Convex side.
 */
...
export interface SendMediaNodeConfig {
   ...
   * `media_url` (see `convex/lib/r2/url.ts`). Optional/dormant: no
   * builder-UI write path sets this yet (Task 5 of the R2 migration is
   * dual-read only — see `docs/superpowers/plans/2026-07-19-r2-media-write-path.md`);
   * `flowsEngine.ts` resolves `media_key ?? media_url` at send time.
   */
  media_key?: string;
...
// ============================================================
// DB-row shapes (read by the ORIGINAL engine via supabaseAdmin — kept
// for parity; the Convex engine reads Doc<"flows">/Doc<"flowNodes">/
// Doc<"flowRuns"> instead, see this file's header comment)
// ============================================================
