...
//
// Convex port of `src/lib/automations/validate.ts` — ported verbatim
// (quote style converted to double-quote, this codebase's `convex/`
// convention) except two import changes:
//   - `validateInteractivePayload` comes from the sibling
//     `../whatsapp/interactive` instead of `@/lib/whatsapp/interactive`.
//   - `AutomationTriggerType` is inlined below (its exact 8-value union,
//     copied from `src/types/index.ts`) instead of importing the whole
//     grab-bag `@/types` module across the convex/src boundary just for
//     one type used in a single function's parameter.
//
...
/**
 * Copied from `src/types/index.ts`'s `AutomationTriggerType` — see this
 * file's header comment for why it's inlined rather than imported.
 */
