 ...
 *
 * Convex port of `src/lib/whatsapp/interactive.ts` — ported verbatim
 * except for one import path (`INTERACTIVE_LIMITS` now comes from the
 * sibling `./metaApi` instead of `./meta-api`) and quote style
 * (double-quote, this codebase's `convex/` convention).
 *
 ...
 *   - the automations/flows engines' "send interactive" steps (send +
 *     persist, via `convex/metaSend.ts`'s `sendInteractive`),
 *   - `convex/lib/automations/validate.ts`'s `send_buttons`/`send_list`
 *     activation checks,
 *   - the message bubble + preview (render) once ported.
 *
 * The field names (`id`/`title`/`description` on buttons/rows) match
 * `metaApi.ts`'s `InteractiveButton` / `InteractiveListRow` /
 ...
 * meta-api senders, but returns a result object so callers (the
 * `sendInteractive` action, activation checks) can surface a clean
 * error *before* the network call rather than turning a bad payload
 * into a 400 from Meta mid-conversation.
 */

import { INTERACTIVE_LIMITS } from "./metaApi";
 ...
 * object rather than throwing so callers can map it to a clean
 * user-facing error.
 *
 * `unknown` in, narrowed here, so it's safe to call straight on a parsed
 * request body / stored step config.
 */
...
/**
 * Short single-line summary used for `conversations.lastMessageText`
